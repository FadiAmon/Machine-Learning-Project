...
class Pre():
    ...
    def Fill_Nan_Values(self, df, Structure):
        ...
        if the values of the column is numeric we fill the average of the values
        else we fill the most frequent value of the values.
        :param df: data frame
        :param Structure: dictionary that contain the struct of the data frame
        :return:nothing
        '''
    ...
    def Binning(self, df, Structure, Discretization_type, NumOfBins):
        '''
        ...
        :return: nothing
        '''
        discrize = Binning(df, NumOfBins)
        Disc = {'Equal frequency': discrize.Equal_Frequency,
                'Equal width': discrize.Equal_Width,
    ...
    def read_structure(self, FileName):
        '''
        The function return a dictionary that contain the struct of the data frame by reading the structure file .
        :param FileName: the file name of the structure file.
        :return: a dictionary that contain the struct of the data frame
        '''
    ...
    def Clean_Data(self, df, Strcture, Discretization_type, NumOfBins):
        '''
        take a data frame and bin it and remove the nan class rows and fill the nan values .
        :param df: data frame
        :param Structure: dictionary that contain the struct of the data frame.
        :param Discretization_type: the type of the wanted discretization.
        :param NumOfBins: number of bins for the discretization.
        ...
        self.Binning(df, Strcture, Discretization_type, NumOfBins)
...
class Process(Pre):
    '''
    This class runs the algorithm (our implementation) for saving the model and building the model.
    '''
    ...
    def Build_Model(self, Path, Algorithm, train):
        ...
        if Algorithm == 'naive bayes classifier (our)':
            return NaiveBayes.NaiveBayes(Path, train)
    ...
    def Running_Algorithm(self, Path, Algorithm, train, test):
        ...
        '''
        if Algorithm == 'naive bayes classifier (our)':
            prediction_train = NaiveBayes.Testing_model(Path, self.Load_Model(Path), train)
            prediction_test = NaiveBayes.Testing_model(Path, self.Load_Model(Path), test)
...
class BuildAlgorithm(Pre):
    '''
    This class runs the algorithms (libraries implementation) and for saving the model and building the model.
    '''
    ...
    def Run(self, Algorithm, train, test, Path, n_neighbors=2):
        '''
        Runs the given algorithm for the train and test files and return the results .
        :param Path: the path of the files
        :param Algorithm: the wanted algorithm
        :param train: the train data frame
        :param test: the test data frame
